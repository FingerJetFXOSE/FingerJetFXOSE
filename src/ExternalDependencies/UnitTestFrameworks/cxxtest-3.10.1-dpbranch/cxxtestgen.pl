...
sub scanInputFile($) {
  ...
  while (defined($line = <FILE>)) {
    if ($comment = $comment && !($line =~ m/^\s*\*\/\s*$/) || $line =~ m/^\s*\/\*\s*$/) {
      next;
    }
    ...
    if ( $suite ) {
      if ( lineBelongsToSuite( $suite, $., $line ) && !($suite->{'generated'} & 8) ) {
...
sub scanLineForSuiteStart($$$) {
  ...
  } elsif ( $line =~ m/\bclass\s+(\w+)\s*:\s*public\s+((::)?\s*CxxTest\s*::\s*)?test_suite<\1>/ ) {
    startSuite( $1, $fileName, $lineNo, 8, "" );
  } elsif ( $line =~ m/\bTEST_SUITE\s*\(\s*(\w+)\s*\)/ ) {
    startSuite( $1, $fileName, $lineNo, 8, "" );
  } elsif ( $line =~ m/\bCXXTEST_SUITE\s*\(\s*(\w*)\s*\)/ ) {
...
sub startSuite($$$$) {
  my ($name, $file, $line, $generated, $basename) = @_;
  closeSuite();
# print "startSuite() : $name, $file, $line, $generated, $basename\n";
  $suite = { 'name' => $name,
             'file' => $file,
             'line' => $line,
             'generated' => $generated,
...
sub closeSuite() {
# print "closeSuite()";
  if ( $suite && (scalar @{suiteTests()} || $suite->{'generated'} & 2 || $suite->{'generated'} & 8)) {#defined($suite->{'basename'})
#   print " : $suite->{'name'}, $suite->{'file'}, $suite->{'line'}, $suite->{'generated'}, $suite->{'basename'}";
    verifySuite();
    rememberSuite();
  }
# print "\n";
...
sub writePreamble() {
  ...
  print "#include <cxxtest/TestSuiteEx.h>\n";
...
sub writeSuites() {
  foreach (@suites) {
    ...
    if ( $suite->{'generated'} & 1 ) { generateSuite(); }
    if ( $suite->{'generated'} & 4 ) {
      next;
    }
    if ( !($suite->{'generated'} & 8) ) {
...
sub writeTestDescription() {
  ...
  printf "\nstatic class $class : public CxxTest::RealTestDescription {\n";
  printf "public:\n";
  $noStaticInit or
...
sub writeSuiteDescription() {
  dynamicSuite() ? writeDynamicDescription() : $suite->{'generated'} & 8 ? writeSuite_Instance() : writeStaticDescription() ;
}
...
sub writeSuite_Instance() {
  printf "CxxTest::suite_instance<%s> %s_suite", suiteName(), suiteName();
  if ( !$noStaticInit ) {
    printf "(%s, %s, \"%s\")", fileString(), $suite->{'line'}, suiteName();
  }
  print ";\n";
}
...
sub writeInitialize() {
  ...
  foreach (@suites) {
    ...
    if ( dynamicSuite() ) {
      ...
      if ( $suite->{'generated'} & 8 ) {
        printf "  %s_suite.initialize(%s, %s, \"%s\");\n", suiteName(), fileString(), $suite->{'line'}, suiteName();
      } else {
