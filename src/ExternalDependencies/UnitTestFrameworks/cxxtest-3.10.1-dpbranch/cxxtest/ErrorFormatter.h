...
namespace CxxTest
{
    ...
    public:
        ...
        virtual void flush() {};
        virtual OutputStream &operator << (size_t /* size */) { return *this; }
    ...
    private:
        ...
        void reportTest( void )
        {
            if( _reported )
                return;
            (*_o) << "In Suite : \"" << tracker().suite().suiteName() << "\", Test : \"";
            const char * begin = tracker().test().testName(), * end = begin;
            for (; *end; ++end);
            end -= 2;
            for (; end > begin; --end) if (end[0] == ':' && end[1] == ':') break;
            if (end > begin) {
              for (; begin < end; ++begin) {
                char s[2] = {};
                s[0] = *begin;
